...
//    fileprivate let imageView = UIImageView(image: #imageLiteral(resourceName: "lady5c"))
    ...
    @objc fileprivate func handleTap(gesture: UITapGestureRecognizer) {
        ...
        //if tap is greater den half the width move to next image
...
//        setupBarsStackView()
    ...
    //layer needs z index so it is set before label is set
    fileprivate func setupGradientLayer() {
        gradientLayer.colors = [UIColor.clear.cgColor, UIColor.black.cgColor]
        gradientLayer.locations = [0.5, 1.1]
//        gradientLayer.frame = self.frame
//        self.frame is actually zero frame at initialization
    ...
    @objc fileprivate func handlePan(gesture: UIPanGestureRecognizer) {
        switch gesture.state {
            //remove all animations before setting them
        case .began:
    ...
    fileprivate func handleChanged(_ gesture: UIPanGestureRecognizer) {
        ...
        //we convert radians  to degrees of the translation done and divide by 20 to make it appear really slow . we rotate and translate at the same time 
